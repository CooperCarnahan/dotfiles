...
- [ ] **Step 5c: Enable hyprmoncfgd on fresh machines**

Discovered during execution. `hyprmoncfgd` is enabled on this machine only because it was
enabled by hand; nothing in the repo would reproduce that on a new install, so a fresh
machine would install the tool but never run the daemon — no auto-switching at all.

The same AUR script already manages a user unit (`systemctl --user disable --now
mako.service`), so follow that precedent. After the package loop, add:

```bash
# hyprmoncfg's hotplug/lid daemon. Enabled here because nothing else would start
# it on a fresh machine, leaving monitor profiles inert.
systemctl --user enable --now hyprmoncfgd.service 2>/dev/null || true
```

- [ ] **Step 5d: Make the monitors require fault-tolerant**

Discovered during execution, and the most serious of the three gaps. `hyprland.lua` now does
`require("monitors")` unconditionally, but `~/.config/hypr/monitors.lua` is chezmoi-ignored
and is only ever created when hyprmoncfg applies a profile. On a fresh machine, `chezmoi
apply` therefore produces a config that requires a module that does not exist yet, and
Hyprland fails to load with a module-not-found error — a chicken-and-egg break, since
hyprmoncfg cannot run until the session starts.

In the chezmoi source `dot_config/hypr/hyprland.lua`, make the require tolerant:
...
-- pcall: on a fresh machine the file does not exist until hyprmoncfgd applies a
-- profile. Hyprland then falls back to auto-detected outputs for that one boot
-- instead of failing the whole config load.
pcall(require, "monitors")
```

Verify afterwards that the live config still loads: `hyprctl reload` must return `ok` and
`hyprctl configerrors` must stay empty.
